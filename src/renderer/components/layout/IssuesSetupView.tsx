...
function buildInstallPrompt(env: Environment, app: ReportingApp): string {
  ...
  const parts = [
    ...
    `(3b) For PHP / Laravel only — CRITICAL, do not skip: when you set auto_register => false in Alby::init() (which you should, so the SDK doesn't fight Laravel's own exception handler), the SDK installs NOTHING automatically. Laravel will catch every uncaught exception, log it, render a 500 page, and Alby will never see it. You MUST manually do TWO things or the integration is silently broken:`,
    `(3b-i) Forward Laravel's unhandled exceptions to Alby::captureException(). Laravel 11+ (bootstrap/app.php with ->withExceptions(...)): add $exceptions->report(function (\\Throwable $e) { \\Alby\\Report\\Alby::captureException($e); }); inside the withExceptions closure. Laravel ≤10 (app/Exceptions/Handler.php): inside the register() method add $this->reportable(function (\\Throwable $e) { \\Alby\\Report\\Alby::captureException($e); });. Without this hook, Laravel's handler swallows the exception before it can reach Alby and /test pages that throw 500s will never appear in the Alby issues list — this is the #1 silent-failure mode of the Laravel integration.`,
    `(3b-ii) The alby/report SDK uses a buffering CurlTransport that only POSTs events when flush() is called; with auto_register => false nothing calls flush() for you. Add the shutdown-time flush in bootstrap/app.php right after the init call: register_shutdown_function(static function (): void { \\Alby\\Report\\Alby::flush(2000); });. Without this, events captured during a real HTTP request die with the PHP process and never reach Alby.`,
    `(3b-iii) Verify BOTH steps by triggering a real 500 — add a temporary route like Route::get('/alby-test', fn () => throw new \\RuntimeException('Alby detector test event — post-install verification')); hit it in the browser, confirm a 500 is rendered, then confirm the event shows up in the Alby Issues tab within a few seconds. If the 500 renders but no Alby event appears, step 3b-i is missing. If the event appears on CLI but not on HTTP, step 3b-ii is missing. Remove the temporary route only after both succeed. Skip step 3b entirely only if you explicitly set auto_register => true (not recommended for Laravel because the SDK will wrap PHP's set_exception_handler underneath Laravel's handler and you'll still miss exceptions Laravel rescues in its HTTP kernel).`,
